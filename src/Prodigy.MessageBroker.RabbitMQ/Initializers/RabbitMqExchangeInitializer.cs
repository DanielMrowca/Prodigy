...
namespace Prodigy.MessageBroker.RabbitMQ.Initializers
{
    /// <summary>
    ...
    public class RabbitMqExchangeInitializer : IInitializer
    {
        ...
        public Task InitializeAsync()
        {
            ...
            using (var channel = _connectionFactory.GetConnection().CreateModel())
            {
                ...
                {
                    if (attribute is null || string.IsNullOrWhiteSpace(attribute.Exchange) || (attribute.Exchange.Equals(_options.Exchange?.Name, StringComparison.InvariantCultureIgnoreCase) && _options.Exchange?.Declare == true))
                        continue;
